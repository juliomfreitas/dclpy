    ...
    def raise_only_exception(self, fact):
        raise DivergenceException(
            ...
            message="Only %s can access %s, but %s did" % (
                self.mod, self.mod_target, fact['sender']
            )
        )
        ...
        if fact['sender']:
            if super(MustCreate, self).check(fact):
                raise ResolvedAbsenceException(
                    fact, self, "%s created %s" % (self.mod, self.mod_target)
                )
    ...
    def check(self, fact):
        if fact['sender']:
            if super(MustAccess, self).check(fact):
                raise ResolvedAbsenceException(
                    fact, self, "%s accessed %s" % (self.mod, self.mod_target)
                )
    ...
    def check(self, fact):
        if fact['sender']:
            if super(MustInherit, self).check(fact):
                raise ResolvedAbsenceException(
                    fact, self, "%s inherits from %s" % (self.mod, self.mod_target)
                )
